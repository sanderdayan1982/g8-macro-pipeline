#!/bin/zsh
# nzd_local_run.sh — v1.3 (lote 1 fiabilidad, 2026-09-24): fetch RBNZ B2 (NZD) + SNB (CHF) + BoJ (TONA)
# en el Mac (IP residencial) y publicación por familias con push_nzd_to_github.py v2.0.
...
mkdir -p data logs state
{
  ...
  /usr/bin/python3 push_nzd_to_github.py --fetch-status "nzd=$rc_nzd,chf=$rc_chf,tona=$rc_tona"; rc_push=$?
